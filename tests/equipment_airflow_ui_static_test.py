...
class EquipmentAirflowUiStaticTests(unittest.TestCase):
    def test_image_only_rotated_ec_reports_retry_ocr_orientation(self):
        ...
        self.assertIn("if (EcOcrQuality(bestText) >= 200)", ENERGY)
        self.assertNotIn("if (bestText.Trim().Length >= 500)", ENERGY)
        ...
        self.assertIn("pages[i].GetImages()", ENERGY)
        self.assertIn("image.TryGetPng(out byte[] png)", ENERGY)
        self.assertIn("OcrPngEc(png, engine, HasTime)", ENERGY)
        self.assertIn("CreatePdfImageInkBitmap(loaded)", ENERGY)
        self.assertIn("rendered[i] = LooksLikeUsefulEcOcrText(candidate)", ENERGY)
        self.assertIn("BestEcPageText(texts[i], parts)", ENERGY)
        self.assertNotIn("LooksLikeUsefulEcOcrText(sb.ToString())", ENERGY)
